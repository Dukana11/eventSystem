...
export default function EventTypeTable({refreshTrigger}:EventTypeProps) {
...
// }, [category]);
  ...
  function getCategory() {
    axios.get("http://localhost:3001/api/eventType")
      .then((response) => {
  ...
  function handleEditSubmit(e: React.FormEvent) {
    ...
    axios
      .put(`http://localhost:3001/api/editEventType/${selectedCategory.ID}`, { name: editedName })
      .then((response) => {
        if (response.data.success) {
          alert("Төрөл шинэчлэгдлээ.");
          closeModal();
  ...
  function deleteCategory(id: number) {
    const confirmed = confirm("Та энэ төрлийг устгахдаа итгэлтэй байна уу?");
    if (!confirmed) return;
  
    axios
      .delete(`http://localhost:3001/api/deleteEventType/${id}`)
      .then((response) => {
        if (response.data.success) {
