...
import EventStatusTable from "@/components/tables/EventStatus";

export default function EventStatePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post("http://localhost:3001/api/addEventStatus", { name });
  ...
  return (  
    <div>
      <PageBreadcrumb pageTitle="Event status" />
      <ComponentCard title="Эвентийн төлөв">
        <button
          onClick={openModal}
          type="button"
        ...
        <EventStatusTable refreshTrigger={refreshKey} />
      </ComponentCard>
